...
import sys
import os
sys.path.insert(0, '/home/runner/work/pdpipe/pdpipe/src')
...
def test_user_precondition_error():
    ...
    try:
        ...
        if "column x" in str(e).lower() or "user-provided precondition" in str(e).lower():
            print("✓ SUCCESS: User precondition error message is correct!")
            return True
        else:
            print("✗ FAILURE: Error message does not indicate user precondition failure")
            print("Expected: Message about column 'x' not found (user precondition)")
            print("Got:", str(e))
            return False
...
def test_stage_precondition_error():
    ...
    pline = pdp.PdPipeline([
        pdp.FreqDrop(2, 'a')  # column 'a' doesn't exist
    ])
