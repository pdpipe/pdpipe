...
class SklearnTransform(ColumnsBasedPipelineStage):
    ...
    subset of the input dataframe columns and reinserts the transformed matrix
    at the position of the first selected column. Columns not selected for
    transformation are passed through unchanged and retain their relative
    order.
    ...
    **kwargs : extra keyword arguments
    ...
    >>> stage = pdp.SklearnTransform(StandardScaler(), ['x', 'y'])
    ...
    def _as_2d_array(res):
        ...
        if res.ndim != 2:
            raise PipelineApplicationError(
                "SklearnTransform expected a 2-dimensional transformer output;"
    ...
    def _resolve_result_columns(self, selected_columns, n_output_columns):
        if self._result_columns is not None:
            if len(self._result_columns) != n_output_columns:
                raise PipelineApplicationError(
                    "SklearnTransform result_columns length must match "
    ...
    def _make_result_df(self, res, X, selected_columns):
        arr = self._as_2d_array(res)
        if arr.shape[0] != len(X.index):
            raise PipelineApplicationError(
                "SklearnTransform transformer output row count must match "
    ...
    def _insert_result(self, X, selected_columns, result_X):
        ...
        if len(set(result_columns)) != len(result_columns):
            raise PipelineApplicationError(
                "SklearnTransform result column labels must be unique."
            )
        colliding_cols = [
        ...
        if colliding_cols:
            raise PipelineApplicationError(
                "SklearnTransform result column labels must not collide with "
    ...
    def _fit_transform(self, X, verbose):
        ...
        try:
            result = self.transformer_.fit_transform(sub_X.values)
            result_X = self._make_result_df(
                result, X, self._columns_to_transform
            )
        except PipelineApplicationError:
            raise
        except Exception as e:
            raise PipelineApplicationError(
                "Exception raised when SklearnTransform applied to columns"
    ...
    def _transform(self, X, verbose):
        sub_X = X[self._columns_to_transform]
        try:
            result = self.transformer_.transform(sub_X.values)
            result_X = self._make_result_df(
                result, X, self._columns_to_transform
            )
        except PipelineApplicationError:
