...
class SklearnColumnTransform(ColumnsBasedPipelineStage):
    """A stage wrapping a matrix-to-matrix scikit-learn transformer.

    This stage applies an arbitrary sklearn-style transformer to a selected
    subset of the input dataframe columns. Same-width outputs replace the
    selected columns at their original positions. Shape-changing outputs are
    reinserted as a single block at the position of the first selected column.
    Columns not selected for transformation are passed through unchanged and
    retain their relative order.

    Parameters
    ----------
    transformer : sklearn.TransformerMixin
        A scikit-learn compatible transformer implementing ``fit_transform``
        and ``transform``.
    columns : single label, list-like or callable, default None
        Column labels in the DataFrame to transform. If columns is None then
        all columns are transformed, except those given in the exclude_columns
        parameter. Alternatively, this parameter can be assigned a callable
        returning an iterable of labels from an input pandas.DataFrame. See
        `pdpipe.cq`.
    exclude_columns : single label, list-like or callable, default None
        Label or labels of columns to exclude from transformation.
    result_columns : single label or list-like, default None
        Labels for the transformed matrix columns. If None and the transformer
        output has the same number of columns as the selected input subset, the
        selected input labels are reused. If None and the output width differs,
        labels are generated by formatting ``lbl_format`` with output column
        indices. Result labels are fixed on fit and reused on transform.
    lbl_format : str, default 'sklearn{}'
        Format string used to generate labels for shape-changing transformer
        output when result_columns is None.
    **kwargs : extra keyword arguments
        PdPipelineStage valid keyword arguments are used to override this
        stage's defaults. All other extra keyword arguments are forwarded to
        the wrapped transformer with ``set_params`` before fitting.

    Attributes
    ----------
    transformer_ : sklearn.TransformerMixin
        The fitted transformer clone used by the stage.
    ...
    >>> df = pd.DataFrame(
    ...     [[1, 10, 'a'], [2, 20, 'b']], columns=['x', 'y', 'z'])
    >>> stage = pdp.SklearnColumnTransform(StandardScaler(), ['x', 'y'])
    >>> stage(df).columns.tolist()
    ['x', 'y', 'z']
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if result_columns is not None:
            self._result_columns = list(
                _interpret_columns_param(result_columns)
            )
        self._lbl_format = lbl_format
        if lbl_format is None:
            self._lbl_format = "sklearn{}"
        self._kwargs = kwargs.copy()
        super_kwargs = {
            "columns": columns,
            "exclude_columns": exclude_columns,
            "desc_temp": f"Transform columns {{}} with {transformer}",
            "none_columns": "all",
        }
        valid_super_kwargs = super()._init_kwargs()
        for key in kwargs:
    ...
    def _resolve_result_columns(self, selected_columns, n_output_columns):
        if self._result_columns is not None:
            if len(self._result_columns) != n_output_columns:
                raise PipelineApplicationError(
                    "SklearnColumnTransform result_columns length must match "
                    "transformer output width. Got "
                    f"{len(self._result_columns)} labels for "
                    f"{n_output_columns} output columns."
                )
            return self._result_columns
        if n_output_columns == len(selected_columns):
    ...
    def _make_result_df(self, res, X, selected_columns, result_columns=None):
        ...
        return pd.DataFrame(data=arr, index=X.index, columns=result_columns)

    def _insert_result(self, X, selected_columns, result_X):
        if len(selected_columns) == 0:
            return X
        passthrough_cols = [
            col for col in X.columns if col not in selected_columns
        ]
        result_columns = list(result_X.columns)
        if len(set(result_columns)) != len(result_columns):
            raise PipelineApplicationError(
                "SklearnColumnTransform result column labels must be unique."
            )
        colliding_cols = [
            col for col in result_columns if col in passthrough_cols
        ]
        if colliding_cols:
            raise PipelineApplicationError(
        ...
        if self._replace_selected:
            selected_to_result = dict(zip(selected_columns, result_columns))
            post_X = X.copy().drop(columns=selected_columns)
            for selected_col in selected_columns:
                result_col = selected_to_result[selected_col]
                post_X[result_col] = result_X[result_col]
            post_columns = []
            for col in X.columns:
                post_columns.append(selected_to_result.get(col, col))
            return post_X.loc[:, post_columns]
        first_loc = min(X.columns.get_loc(col) for col in selected_columns)
        before = [
            col
            for col in passthrough_cols
            if X.columns.get_loc(col) < first_loc
        ]
        after = [
            col
            for col in passthrough_cols
        ...
        return pd.concat([X[before], result_X, X[after]], axis=1)

    def _validate_transform_columns(self, X):
        missing_cols = [
            col for col in self._columns_to_transform if col not in X.columns
        ]
        if missing_cols:
            raise PipelineApplicationError(
    ...
    def _fit_transform(self, X, verbose):
        self._columns_to_transform = list(self._get_columns(X, fit=True))
        sub_X = X[self._columns_to_transform]
        self.transformer_ = clone(self.transformer)
        self.transformer_ = self.transformer_.set_params(**self._kwargs)
        try:
            result = self.transformer_.fit_transform(sub_X)
            result_arr = self._as_2d_array(result)
            self._result_columns_ = self._resolve_result_columns(
                self._columns_to_transform, result_arr.shape[1]
            )
            self._replace_selected = result_arr.shape[1] == len(
    ...
    def _transform(self, X, verbose):
        ...
        sub_X = X[self._columns_to_transform]
        try:
            result = self.transformer_.transform(sub_X)
            result_X = self._make_result_df(
                result,
                X,
                self._columns_to_transform,
