...
def _minmax_scaler_df():
    return pd.DataFrame([[2, 3], [5, 7]], columns=["a", "b"])
...
def test_scaler_no_y():
    ...
    expected_res = pd.DataFrame(
        [[-1.264911, -0.632456], [0.632456, 1.264911]], columns=["a", "b"]
    )
    ...
    expected_res = pd.DataFrame([[0, 0.2], [0.6, 1]], columns=["a", "b"])
...
def test_fittable():
    ...
    res = pipeline.apply(_standard_scaler_df())
    expected_res = pd.DataFrame([[0, 0.2], [0.6, 0.8]], columns=["a", "b"])
    pd.testing.assert_frame_equal(res, expected_res)
