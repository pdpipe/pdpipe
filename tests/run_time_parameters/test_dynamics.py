...
def test_scaler_with_y():
    ...
    res_x, res_y = pipeline.apply(df_standard_scaler_x, df_standard_scaler_y)
    ...
    res_x, res_y = pipeline.apply(df_minmax_scaler_x, df_minmax_scaler_y)
...
def test_scaler_no_y():
    ...
    assert scaler.scaler == 'MinMaxScaler'
    pd.testing.assert_frame_equal(res, expected_res)
    # in terminal pytest -m runtime (use marks from pytest package)
    # cover lines in core.py
...
def test_transform():
    ...
    pipeline.apply(_standard_scaler_df())
    assert scaler.scaler == 'MinMaxScaler'
