...
def _original_code():
    ...
    pline = pdp.PdPipeline(
        [
            ...
            pdp.ApplyByCols(["date", "shelf_date", "end_of_season"], pd.to_datetime),
            pdp.ApplyToRows(
            ...
            pdp.AdHocStage(lambda df: df[df["days_on_counter"] <= df["life_cycle"]]),
    ...
    salesdata_cumitems = (
        salesdata[["SKC", "date", "sales", "passenger_flow", "plus_purchase"]]
        .sort_values(by=["SKC", "date"])
        .groupby(["SKC"])
        .cumsum()
    )
    ...
    salesdata["total_passenger_flow"] = salesdata_cumitems["total_passenger_flow"]
    salesdata["total_plus_purchase"] = salesdata_cumitems["total_plus_purchase"]
