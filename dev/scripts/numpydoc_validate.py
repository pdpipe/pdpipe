...
HIDE_ERROR_REGEX = '|'.join([
    ':{}:'.format(code)
    for code in HIDE_ERROR_CODES_LIST
])
...
SOFT_ERROR_REGEX = '|'.join([
    ':{}:'.format(code)
    for code in SOFT_ERROR_CODES_LIST
])
...
NAME_SKIP = [
    'tqdm', 'skintegrate', 'count', 'index',
    'Any', 'Callable', 'Optional', 'List', 'Tuple', 'Set', 'set', 'Union',
    'Iterable',
    # pdpipe types that are not objects that should be checked
    'SeriesOperandTypesTuple', 'ColumnsParamType', 'ColumnLabelsType',
    # pdpipe globals we can skip
    'LOAD_STAGE_ATTRIBUTES', 'POS_ARG_MISMTCH_PAT',
]
